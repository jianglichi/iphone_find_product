...
- (void)SearchItems :(NSInteger *)pages
{
	...
	[yahoo getItem:searchtext :self.setbutton :self];
...
- (IBAction) search_result :(NSMutableArray *)returnlistItems
{
	...
	if(rowcount>0 && rowcount<1000){
		search_msg_yahoo.text  = [NSString stringWithFormat:@"%d/%@",nowpages,[listItems objectAtIndex:0]];
	}else{
		search_msg_yahoo.text  = @"";
	}
	
		//頁數
		[self set_search_bar:2];
		
		[tablemyview_yahoo reloadData];
	if(rowcount>0 && rowcount<1000)
		[tablemyview_yahoo scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:0 inSection:0] atScrollPosition:UITableViewScrollPositionTop animated:NO];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	NSString *CellIdentifier = @"yahooCell";
